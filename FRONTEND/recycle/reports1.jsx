...
const exportToPDF = (title, columns, rows, propertyLabel = 'All Properties', userName = '') => {
  const doExport = (logoDataUrl) => {
    ...
    // ── Header area ─────────────────────────────────────────────────────────
    const headerHeight = userName ? 58 : 50
    ...
    // Logo (top left)
    if (logoDataUrl) {
      doc.addImage(logoDataUrl, 'PNG', 12, 8, 38, 14)
    } else {
      doc.setFontSize(16)
    ...
    // Property + Report title (below logo, left side)
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(9)
    doc.setTextColor(100, 116, 139)
    doc.text('Property: ', 14, 32)
    const propLabelWidth = doc.getTextWidth('Property: ')
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(15, 23, 42)
    doc.text(propertyLabel, 14 + propLabelWidth, 32)
    ...
    // Right side: Generated on + Prepared for
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(8)
    doc.setTextColor(100, 116, 139)
    doc.text('GENERATED ON', pageWidth - 14, 14, { align: 'right' })
    doc.setFontSize(9)
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(15, 23, 42)
    doc.text(new Date().toLocaleString('en-KE', { dateStyle: 'medium', timeStyle: 'short' }), pageWidth - 14, 21, { align: 'right' })

    if (userName) {
      doc.setFont('helvetica', 'normal')
      doc.setFontSize(8)
      doc.setTextColor(100, 116, 139)
      doc.text('PREPARED FOR', pageWidth - 14, 32, { align: 'right' })
      doc.setFontSize(9)
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(15, 23, 42)
      doc.text(userName, pageWidth - 14, 39, { align: 'right' })
    }
